...
execute if score active raining_water matches 1 at @a align xyz positioned ~0.5 255 ~0.5 unless entity @e[tag=its_raining_barrier,distance=..0.5] run summon minecraft:armor_stand ~ ~ ~ {Invisible:1b,Marker:1b,Tags:["its_raining_barrier"]}
execute if score active raining_water matches 1 at @a align xyz run fill ~-0.5 255 ~-0.5 ~1.5 255 ~1.5 minecraft:barrier
